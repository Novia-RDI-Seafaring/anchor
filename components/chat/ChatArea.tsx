...
export const ChatArea: React.FC<ChatAreaProps> = ({ messages, isEmpty }) => {
  if (isEmpty) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center text-neutral-800 p-6 animate-in fade-in duration-500">
  ...
  return (
    <div className="flex-1 overflow-y-auto p-4 space-y-6 scroll-smooth">
      {messages.map((msg) => (
        ...
        >
          {msg.role === 'assistant' && (
            <div className="w-6 h-6 rounded-full bg-teal-600 flex items-center justify-center flex-shrink-0 mt-0.5 shadow-sm text-white">
              <Bot size={14} />
            </div>
          )}
