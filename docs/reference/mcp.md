...
| Canvas | `canvas_list_workspaces`, `canvas_get_state`, `canvas_add_node`, `canvas_update_node`, `canvas_add_edge`, `canvas_node_types`, `canvas_set_review_mode`, `canvas_snapshot` |
| Proposal sets | `canvas_propose_set`, `canvas_add_to_proposal_set`, `canvas_list_proposal_sets`, `canvas_review_proposal_set` - group what an agent added in one go so a human rules on the batch, not each element (HTTP `/api/workspaces/{slug}/proposal-sets`, CLI `anchor canvas propose-set\|add-to-set\|proposal-sets\|review-set`) |
