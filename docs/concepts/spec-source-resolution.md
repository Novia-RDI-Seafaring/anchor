...
core does not know PDF stores, table cells or spec rows.
...
Without a producer preparer, workspace mutation retains its generic behavior.

The PDF preparer calls the existing strict `enrich_spec_row_source_refs`:
...
- Certified-pair metadata is consumed and checked when supplied. Enrichment
  does not invent a new verification status or require new source fields.
...
Unchanged output is not a verified-grounding verdict. Claim edits can retain
old caller citations under existing merge semantics. This change does not add
stale/unverified UI states, repair historical claims, or implement G7.
