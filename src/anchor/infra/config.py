...
_POSIX_ENV_VAR = re.compile(r"\$(\w+)|\$\{([^}]+)\}")


def expand_env_vars(value: str) -> str:
    """Expand both Windows ``%VAR%`` and POSIX ``$VAR`` environment syntax."""
    ...
    return _POSIX_ENV_VAR.sub(replace, expanded)
...
class AnchorConfig(BaseSettings):
    ...
    def settings_customise_sources(
        ...
        if layers is not None:
            ...
            # the environment's own .env into os.environ. Do not also read a
            # cwd .env here; that would let an unrelated shell/project secret
            # override the named environment's endpoint or models.
