...
third-party OIP manifests remain data contracts; this module never imports
arbitrary code from them.
"""
...
class ExtensionRuntimeStatus:
    """Availability of one bundled extension in a project runtime."""
...
class ExtensionHost:
    """Discover manifests and start only bundled extension runtimes."""
    ...
    def discover(
        ...
        *,
        ...
        locations = (("system", system_producers_dir()),)
        if self.data_dir is not None:
            locations += (("project", project_producers_dir(self.data_dir)),)
        for source, directory in locations:
            ...
            for path in sorted(directory.glob("*.json")):
                ...
                if manifest is not None:
                    found[source].append(manifest)
        return found

    def third_party_manifest_paths(self) -> list[Path]:
        """Return registered third-party manifest paths in stable order."""
        found: list[Path] = []
        directories = [system_producers_dir()]
        if self.data_dir is not None:
            directories.append(project_producers_dir(self.data_dir))
    ...
    def start(
        ...
        *,
        ...
        if name == "cad":
            from anchor.extensions.anchor_cad import extension as cad_extension

            return cad_extension.build_service(self.data_dir, bus)
        if name == "fmu":
            from anchor.extensions.anchor_fmus import extension as fmu_extension

            return fmu_extension.build_service(self.data_dir, bus)
        if name == "sysml":
            if workspace is None:
                raise RuntimeError("ExtensionHost.start('sysml') requires WorkspaceService")
            from anchor.extensions.anchor_sysml import extension as sysml_extension

            return sysml_extension.build_service(
                self.data_dir,
                bus,
                workspace=workspace,
            )
    ...
    def start_bundled(
        ...
        *,
        ...
        """Start required bundled modules and record optional FMU failure."""
        if self.data_dir is None:
        ...
        try:
            ...
            status[fmu_extension.NAME] = ExtensionRuntimeStatus(
                name=fmu_extension.NAME,
                source="bundled",
                available=True,
            )
        except Exception as exc:  # noqa: BLE001 - FMU is an optional extra
            status[fmu_extension.NAME] = ExtensionRuntimeStatus(
                name=fmu_extension.NAME,
                source="bundled",
                available=False,
