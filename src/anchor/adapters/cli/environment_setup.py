"""Create and configure Anchor environments through the CLI.

This module owns provider selection, non-secret env.toml rendering, API key
guidance, and environment reporting. Project-folder creation stays in
``anchor.adapters.cli.init``.
"""
...
def _choose(message: str, items: list, render, *, interactive: bool):
    ...
    if questionary is not None:
        answer = questionary.select(
            ...
            instruction="(up/down to move, Enter to choose)",
            qmark="?",
            pointer=">",
        ).ask()
...
def environment_for_project_init(
    env_name: str,
    *,
    provider: str | None,
    embed_model: str | None,
...
def _report(env_name: str, prov: Provider) -> None:
    ...
    if prov.does_vision:
        ...
        if needs_key:
            if has_anchor_key:
                key_state = "ANCHOR_OPENAI_API_KEY detected ok"
            elif prov.key == "openai" and personal:
                key_state = "using OPENAI_API_KEY ok"
            else:
