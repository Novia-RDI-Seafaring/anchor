...
export const MainContent: React.FC = () => {
  ...
  const sourcesByFile = Array.from(
    sourceNodes.reduce((acc: Map<string, { filename: string; pages: number[]; count: number }>, node: any) => {
      if (!node?.filename) {
        return acc;
      }

      const current = acc.get(node.filename) || { filename: node.filename, pages: [], count: 0 };
      const nextPages = node.page ? Array.from(new Set([...current.pages, node.page])).sort((a, b) => a - b) : current.pages;
      acc.set(node.filename, {
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {sourcesByFile.map((source: any) => (
                  <AgCard key={source.filename} className="p-4 hover:shadow-md transition-all duration-200 cursor-pointer group border-neutral-200/60 dark:border-neutral-800">
                    ...
                    <h3 className="font-medium text-neutral-900 dark:text-white truncate" title={source.filename}>
                      {source.filename}
                    </h3>
                    <p className="text-xs text-neutral-500 dark:text-neutral-400 mt-1 line-clamp-2">
                      {source.pages.length > 0
                        ? `Referenced on page${source.pages.length > 1 ? 's' : ''}: ${source.pages.join(', ')}`
                        : 'Referenced in the canvas evidence graph.'}
                    </p>
                  </AgCard>
                ))}
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <AgCard className="p-4 border-neutral-200/60 dark:border-neutral-800 opacity-50">
                  ...
                  <p className="text-xs text-neutral-500 dark:text-neutral-400 mt-1">
                    Ask a technical question to populate source evidence on the canvas.
                  </p>
                </AgCard>
              </div>
            )}
