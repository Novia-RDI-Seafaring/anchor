...
try:
    ...
    instrument_ketju(service_name="rag-agent", enable_logfire=False, instrument_pydantic_ai=True, instrument_llamaindex=True, quiet=False)
except Exception as e:
...
def setup_rag(ingest: bool = False):
    ...
    persist_dir = Path("/Users/toffe/dev/ai/novia/anchor-kb-ui-RAG/data/tmp_rag")
...
from ketju.agents.pydantic_ai.agent import agui_agent, create_agent, Deps
agent = create_agent(model="gpt-4o-mini", deps_type=Deps)
folder, files = get_folder_and_files()
rag.ingest(files)
rag_agent_app = agent.to_ag_ui(deps=Deps(rag_engine=rag, ingested_files=[str(file) for file in files]]))
