...
class ConversationStore:
    ...
    async def list_conversations(self) -> List[Dict[str, Any]]:
        """Return all conversations ordered newest-first (no messages payload)."""
    ...
    async def create_conversation(self, conversation_id: str, title: str = "New Conversation") -> Dict[str, Any]:
        pool = await self._pool()
        async with pool.acquire() as conn:
            row = await conn.fetchrow(f"""
                INSERT INTO "{self._schema}".conversations (id, title)
                VALUES ($1, $2)
    ...
    async def delete_conversation(self, conversation_id: str) -> bool:
        ...
        async with pool.acquire() as conn:
            result = await conn.execute(f"""
                DELETE FROM "{self._schema}".conversations WHERE id = $1
            """, conversation_id)
